...
"""
    @dimension(symb, abbr, name, autodocs=false, makepublic=false)
...
If `makepublic=true`, makes the dimension name, abbreviation, and corresponding `Units` and `FreeUnits` type names public when running on Julia ≥ 1.11.
...
"""
macro dimension(symb, abbr, name, autodocs=false, makepublic=false)
    s = Symbol(symb)
    x = Expr(:quote, name)
...
"""
    @derived_dimension(name, dims, autodocs=false, makepublic=false)
...
If `autodocs == true`, docstrings will be automatically generated for these aliases.
If `makepublic=true`, makes the dimension name public when running on Julia ≥ 1.11.
...
"""
macro derived_dimension(name, dims, autodocs=false, makepublic=false)
    uname = Symbol(name,"Units")
    funame = Symbol(name,"FreeUnits")
    name_links = __module__ == Unitful ? "[`Unitful.Quantity`](@ref), [`Unitful.Level`](@ref)" : "`Unitful.Quantity`, `Unitful.Level`"
    unit_links = __module__ == Unitful ? "[`Unitful.Units`](@ref)" : "`Unitful.Units`"
...
"""
    @refunit(symb, abbr, name, dimension, tf, autodocs=false, makepublic=false)
...
If `makepublic=true`, makes the units names public when running on Julia ≥ 1.11.
...
"""
macro refunit(symb, abbr, name, dimension, tf, autodocs=false, makepublic=false)
    expr = Expr(:block)
...
"""
    @unit(symb, abbr, name, equals, tf, autodocs=false, makepublic=false)
...
If `makepublic=true`, makes the unit name public when running on Julia ≥ 1.11.
...
"""
macro unit(symb, abbr, name, equals, tf, autodocs=false, makepublic=false)
    expr = Expr(:block)
    n = Meta.quot(Symbol(name))
...
"""
    @prefixed_unit_symbols(symb, name, user_dimension, basefactor, autodocs=false, makepublic=false)
...
"""
macro prefixed_unit_symbols(symb, name, user_dimension, basefactor, autodocs=false, makepublic=false)
    expr = Expr(:block)
    n = Meta.quot(Symbol(name))
...
Quantity{Float64, 𝐋 𝐓^-1, Unitful.VelocityFreeUnits{(m, s^-1), nothing}}
